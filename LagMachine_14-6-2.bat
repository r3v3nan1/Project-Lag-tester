...
title NETWORK MASTER CONTROL v14.6.3 Beta
...
echo  ^|             [ VERSION : 14.6.3 Beta STABLE ]              ^|
echo  +-----------------------------------------------------------+
echo  ^|  IP      : %my_ip%                                        
...
echo.
if "%turbo_mode%"=="ON" (
    :: STABILITY REFIX: Combine streams using background execution architecture to protect OS
    set /a "total_pings=%win% * %cnt%"
    start /high /min cmd /c "for /L %%t in (1,1,%win%) do start /b ping %my_gw% -n %cnt% -l %psize% -w 1 >nul"
    cls
    echo.
    echo  +-----------------------------------------------------------+
    echo    TURBO PIPELINES COUPLED SUCCESSFULLY
    echo    BACKGROUND PROCESS HANDLING STABILIZED
