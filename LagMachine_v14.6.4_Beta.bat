...
title NETWORK MASTER CONTROL v14.6.4 Beta
...
echo  ^|             [ VERSION : 14.6.4 Beta REFIX ]               ^|
echo  +-----------------------------------------------------------+
echo  ^|  IP      : %my_ip%                                        
...
if "%s_choice%"=="1" set "theme=02" & echo 02>"%theme_file%"
if "%s_choice%"=="2" set "theme=06" & echo 06>"%theme_file%"
if "%s_choice%"=="3" set "theme=09" & echo 09>"%theme_file%"
if "%s_choice%"=="4" set "theme=0F" & echo 0F>"%theme_file%"
if "%s_choice%"=="5" set "theme=0B" & echo 0B>"%theme_file%"
if "%s_choice%"=="6" set "theme=0D" & echo 0D>"%theme_file%"
if "%s_choice%"=="7" set "theme=04" & echo 04>"%theme_file%"
if "%s_choice%"=="8" set "theme=08" & echo 08>"%theme_file%"
...
echo.
echo  +-----------------------------------------------------------+
echo  ^|             SYNCHRONIZING ATTACK TOPOLOGY                ^|
echo  +-----------------------------------------------------------+
echo.
if "%turbo_mode%"=="ON" (
    :: THE REFIX: Generate an explicit string chain executing natively within a localized hidden shell
    set "cmd_chain="
    for /L %%c in (1,1,%win%) do (
        set "cmd_chain=!cmd_chain! start /b ping %my_gw% -n %cnt% -l %psize% -w 1 ^>nul &"
    )
    :: Strip trailing ampersand and deploy as a solid macro execution string
    set "cmd_chain=!cmd_chain:~0,-1!"
    start /high /min cmd /c "!cmd_chain!"
    cls
    echo.
    echo  +-----------------------------------------------------------+
    echo    TURBO CORES COUPLING: SUCCESS
    echo    PIPELINE EXECUTION STRINGS COMPLETED
